...
base,form = MaxPlus.LoadUiType(os.path.join(os.path.dirname(__file__),'ui','progress_bar_main.ui'))
...
def Run(title='Progress', maximum=100):
    try: # PySide
        tool = ProgressBar(parent=MaxPlus.GetQMaxWindow(), title=title, maximum=maximum)
        print "Running with PySide"
    except: # PySide2
        tool = ProgressBar(parent=MaxPlus.GetQMaxMainWindow(), title=title, maximum=maximum)
