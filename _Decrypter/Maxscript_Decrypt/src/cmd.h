...
int CMD_ComType_func(int cmd) {
    ...
    if(!strnicmp(str, "COMP_", 5)) str += 5;
         ...
         if(!stricmp(str, "zlib") || !stricmp(str, "zlib1"))  compression_type = COMP_ZLIB;
    else if(!stricmp(str, "deflate") || !stricmp(str, "inflate")) compression_type = COMP_DEFLATE;
    QUICK_COMP_ASSIGN(LZO1)
    QUICK_COMP_ASSIGN(LZO1A)
    QUICK_COMP_ASSIGN(LZO1B)
    QUICK_COMP_ASSIGN(LZO1C)
    QUICK_COMP_ASSIGN(LZO1F)
    else if(!stricmp(str, "lzo1x") || !stricmp(str, "lzo") || !stricmp(str, "minilzo")) compression_type = COMP_LZO1X;
    ...
    QUICK_COMP_ASSIGN(LZMA_86HEAD)      // 5 bytes + 8 bytes (size) + lzma
    QUICK_COMP_ASSIGN(LZMA_86DEC)       // 1 byte + 5 bytes + lzma
    QUICK_COMP_ASSIGN(LZMA_86DECHEAD)   // 1 byte + 5 bytes + 8 bytes (size) + lzma
    QUICK_COMP_ASSIGN(LZMA_EFS)         // 2 + 2 + x + lzma
    else if(!stricmp(str, "LZMA86HEAD")) compression_type = COMP_LZMA_86HEAD;
    else if(!stricmp(str, "LZMA86DEC")) compression_type = COMP_LZMA_86DEC;
    else if(!stricmp(str, "LZMA86DECHEAD")) compression_type = COMP_LZMA_86DECHEAD;
    else if(!stricmp(str, "LZMAEFS")) compression_type = COMP_LZMA_EFS;
    ...
    else if(!stricmp(str, "hex") || !stricmp(str, "hex2byte")) compression_type = COMP_HEX;
    QUICK_COMP_ASSIGN(BASE64)
    else if(!stricmp(str, "uuencode") || !stricmp(str, "uudecode")) compression_type = COMP_UUENCODE;
    else if(!stricmp(str, "xxencode") || !stricmp(str, "xxdecode")) compression_type = COMP_XXENCODE;
    QUICK_COMP_ASSIGN(ASCII85)
    QUICK_COMP_ASSIGN(YENC)
    else if(!stricmp(str, "COM_LZW_Decompress")) compression_type = COMP_UNLZW;
    else if(!stricmp(str, "milestone_lzw")) compression_type = COMP_UNLZWX;
    ...
    else if(!stricmp(str, "lzxcab") || !stricmp(str, "mslzx")) compression_type = COMP_LZXCAB;
    ...
    else if(!stricmp(str, "uncompress") || !stricmp(str, "compress") || !stricmp(str, "lzw")) compression_type = COMP_UNCOMPRESS;
    QUICK_COMP_ASSIGN(DMC)
    else if(!stricmp(str, "lzh") || !stricmp(str, "lzhuf") || !stricmp(str, "lha")) compression_type = COMP_LZH;
    QUICK_COMP_ASSIGN(LZARI)
    QUICK_COMP_ASSIGN(TONY)
    QUICK_COMP_ASSIGN(RLE7)
    QUICK_COMP_ASSIGN(RLE0)
    ...
    else if(!stricmp(str, "lzf") || !stricmp(str, "fastlz")) compression_type = COMP_LZF;   // "It is possible to use FastLZ as a drop-in replacement for Marc Lehmann's LibLZF."
    ...
    else if(!stricmp(str, "inflate64") || !stricmp(str, "deflate64")) compression_type = COMP_DEFLATE64;
    ...
    else if(!stricmp(str, "hstest_hs_unpack")) compression_type = COMP_HSTEST1;
    else if(!stricmp(str, "hstest_unpackc")) compression_type = COMP_HSTEST2;
    ...
    else if(!stricmp(str, "ppmdi_raw") || !stricmp(str, "ppmdi1_raw")) compression_type = COMP_PPMDI_RAW;
    ...
    else if(!stricmp(str, "huffman")) compression_type = COMP_HUFFMANLIB;
    QUICK_COMP_ASSIGN(SFASTPACKER)
    QUICK_COMP_ASSIGN(SFASTPACKER2)
    else if(!stricmp(str, "dk2") || !stricmp(str, "ea")) compression_type = COMP_DK2;
    QUICK_COMP_ASSIGN(LZ77WII)
    else if(!stricmp(str, "lz77wii_raw") || !stricmp(str, "lz77wii_raw10")) compression_type = COMP_LZ77WII_RAW10;
    ...
    else if(!stricmp(str, "UnPackSLZ")) compression_type = COMP_SLZ;
    ...
    else if(!stricmp(str, "lzs") || !stricmp(str, "mppc")) compression_type = COMP_LZS;
    else if(!stricmp(str, "lzs_big") || !stricmp(str, "mppc_big")) compression_type = COMP_LZS_BIG;
    QUICK_COMP_ASSIGN(COPY)
    QUICK_COMP_ASSIGN(MOHLZSS)
    QUICK_COMP_ASSIGN(MOHRLE)
    else if(!stricmp(str, "yaz0") || !stricmp(str, "szs")) compression_type = COMP_YAZ0;
    QUICK_COMP_ASSIGN(BYTE2HEX)
    QUICK_COMP_ASSIGN(UN434A)
    else if(!stricmp(str, "pack")) compression_type = COMP_GZPACK;
    else if(stristr(str, "zip_dynamic") || !stricmp(str, "zlib_dynamic")) compression_type = COMP_UNZIP_DYNAMIC;
    QUICK_COMP_ASSIGN(ZLIB_NOERROR)
    QUICK_COMP_ASSIGN(DEFLATE_NOERROR)
    else if(!stricmp(str, "ppmdh") || !stricmp(str, "ppmd7")) compression_type = COMP_PPMDH;
    QUICK_COMP_ASSIGN(PPMDH_RAW)
    QUICK_COMP_ASSIGN(RNC)
    QUICK_COMP_ASSIGN(RNC_RAW)
    else if(!stricmp(str, "PAK_explode") || !stricmp(str, "fitd")) compression_type = COMP_FITD;
    ...
    else if(!stricmp(str, "Nitro") || !stricmp(str, "NitroSDK")) compression_type = COMP_NITROSDK;
    ...
    else if(!stricmp(str, "lzpx") || !stricmp(str, "lzpxj")) compression_type = COMP_LZPX;
    ...
    else if(!stricmp(str, "ntcompress30") || !stricmp(str, "lz77wii_raw30"))  compression_type = COMP_NTCOMPRESS30;
    else if(!stricmp(str, "ntcompress40") || !stricmp(str, "lz77wii_raw40"))  compression_type = COMP_NTCOMPRESS40;
    ...
    else if(!stricmp(str, "kwaj") || !stricmp(str, "mslzh"))  compression_type = COMP_KWAJ;
    ...
    else if(!stricmp(str, "lzxcab_delta") || !stricmp(str, "mslzx_delta")) compression_type = COMP_LZXCAB_DELTA;
...
#define CALLDLL_FUNC2(X) \
        switch(argc) { \
            case 0:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc); break; \
            case 1:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0]); break; \
            case 2:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1]); break; \
            case 3:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2]); break; \
            case 4:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3]); break; \
            case 5:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4]); break; \
            case 6:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4], args[5]); break; \
            case 7:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4], args[5], args[6]); break; \
            case 8:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]); break; \
            case 9:  if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]); break; \
            case 10: if(g_int3) __asm__("int3");    ret = X##_call(funcaddr, argc, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9]); break; \
            default: { \
                fprintf(stderr, "\nError: this tool doesn't support all these arguments for the dll functions ("#X")\n"); \
                myexit(QUICKBMS_ERROR_DLL); \
            } \
